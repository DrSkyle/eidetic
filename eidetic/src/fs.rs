...
impl Filesystem for EideticFS {
    ...
    fn read(
        ...
        if let Some(real_path) = self.real_path(inode) {
             ...
             // GATE: Context Bundler is PRO only
             if !self.check_license() {
                 let msg = "# PRO FEATURE LOCK\n\nThe Context Bundler is a Pro feature.\nPlease purchase a license to unlock AI-ready context bundles.\n\n-> https://eidetic.app/pro\n";
             ...
             if let Some(dir_path) = self.real_path(dir_inode) {
                  // Recursively walk and cat code files
                  let mut content = String::new();
                  content.push_str(&format!("# Context for {:?}\n\n", dir_path.file_name().unwrap_or_default()));
                  
                  // Simple non-recursive for safety/speed in prototype, or recursive limit
                  if let Ok(entries) = std::fs::read_dir(&dir_path) {
                      for entry in entries.flatten() {
                          let p = entry.path();
                          if p.is_file() {
                              // Skip binary simplified check
                              let ext = p.extension().unwrap_or_default().to_string_lossy();
                              if ["rs", "toml", "md", "txt", "js", "py", "c", "h", "cpp"].contains(&ext.as_ref()) {
                                  if let Ok(code) = std::fs::read_to_string(&p) {
                                      content.push_str(&format!("## {}\n```{}\n{}\n```\n\n", p.file_name().unwrap().to_string_lossy(), ext, code));
                                  }
                              }
                          }
                      }
